...
class AIConfigDialog:
    ...
    def _build(self, ai):
        ...
        self.num_predict = self._entry(qbox, 6, "最多輸出 token", max(256, int(ai.get("num_predict", 256))), 6, row=3)
    ...
    def _save(self, announce=True):
        ...
        if timeout <= 0 or delay < 0 or num_predict < 256: raise ValueError("AI 回答逾時必須大於 0、最多輸出 token 至少為 256，回答完成後等待不可小於 0")
        parts = self.resolution.get().replace(" ", "").split("×"); w, h = (map(int, parts) if len(parts) == 2 else (None, None))
