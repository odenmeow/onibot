...
class AIConfigDialog:
    ...
    def _build(self, ai):
        ...
        self.num_predict = self._entry(qbox, 6, "最多輸出 token", ai.get("num_predict", 8), 6, row=3)
    ...
    def _save(self, announce=True):
        ...
        if timeout <= 0 or delay < 0 or num_predict <= 0: raise ValueError("AI 回答逾時與最多輸出 token 必須大於 0，回答完成後等待不可小於 0")
        parts = self.resolution.get().replace(" ", "").split("×"); w, h = (map(int, parts) if len(parts) == 2 else (None, None))
